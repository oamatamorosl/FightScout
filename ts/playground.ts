...
(function initPlayground(): void {
  ...
  function inicializarDialogEliminar(): void {
  ...
  function inicializarMenuDesplegable(idToggle: string, idLista: string): void {
    ...
    toggle.addEventListener('click', (evento: MouseEvent) => {
  ...
  function inicializarToasts(): void {
    ...
    function eliminarToast(toast: HTMLElement): void {
      toast.remove();
    }

    function programarEliminacion(toast: HTMLElement): void {
      window.setTimeout(() => eliminarToast(toast), DURACION_MS);
    }

    function agregarToast(tipo: 'success' | 'error', mensaje: string): void {
      // El máximo de 3 se cumple retirando el toast visible más antiguo,
      // nunca bloqueando la interacción para hacer lugar al nuevo.
      const visibles = contenedor!.querySelectorAll<HTMLElement>('.c-toast');
    ...
    contenedor.querySelectorAll<HTMLElement>('.c-toast').forEach(programarEliminacion);
    ...
    if (disparador instanceof HTMLButtonElement) {
      ...
      disparador.addEventListener('click', () => {
        contador += 1;
        const tipo: 'success' | 'error' = contador % 2 === 0 ? 'error' : 'success';
