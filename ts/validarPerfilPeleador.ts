...
(function initValidacionPerfilPeleador(): void {
  ...
  function calcularEdad(fechaNacimiento: string): number | null {
    ...
    if (noCumplioAnioTodavia) edad -= 1;
  ...
  const DIVISIONES_BOXEO: DivisionPeso[] = [
  ...
  const DIVISIONES_MMA: DivisionPeso[] = [
  ...
  const DIVISIONES_POR_DISCIPLINA: Record<string, DivisionPeso[]> = {
  ...
  const validadores: ConfigCampo[] = [
    ...
    },
    {
      id: 'birthDate',
      validar: (valor) => {
        if (valor.trim() === '') return 'Ingresa tu fecha de nacimiento.';
        const edad = calcularEdad(valor);
        if (edad === null) return 'Ingresa una fecha de nacimiento válida.';
    ...
    },
    {
      ...
      validar: (valor) => {
        ...
        if (estatura === null) return 'Ingresa tu estatura.';
        if (estatura < 120) return 'La estatura mínima es 120 cm.';
        if (estatura > 230) return 'La estatura máxima es 230 cm.';
        return null;
      },
    },
    {
      ...
      validar: (valor) => {
        ...
        if (alcance === null) return 'Ingresa tu alcance.';
        if (alcance < 120) return 'El alcance mínimo es 120 cm.';
        if (alcance > 250) return 'El alcance máximo es 250 cm.';
        return null;
      },
    },
    {
      ...
      validar: (valor) => {
        ...
        if (peso === null || peso <= 0) return 'Ingresa un peso natural válido.';
        return null;
      },
    },
    {
    ...
    },
    {
      ...
      validar: (valor, datos) => {
        ...
        if (debut > hoySinHora) return 'La fecha de debut no puede ser futura.';
        return null;
      },
    },
    {
  ...
  function poblarCheckboxes(
    contenedorOpciones: HTMLElement,
    name: string,
    opciones: OpcionCheckbox[],
    conRevalidacion: boolean = true,
  ): void {
    ...
    opciones.forEach((opcion) => {
      ...
      if (conRevalidacion) agregarListenerGrupo(input, name);
  ...
  function sumaKoTko(): number {
    ...
    return Math.max(valorKo ?? 0, 0) + Math.max(valorTko ?? 0, 0);
  }
  ...
  function actualizarFirstRoundKos(): void {
    const firstRoundKosEl = obtenerCampo('firstRoundKos');
    if (!(firstRoundKosEl instanceof HTMLInputElement)) return;
  ...
  function validarCampoTituloLogro(campo: CampoFormulario): string | null {
    return campo.value.trim() === '' ? 'Ingresa el nombre del logro.' : null;
  }

  function validarCampoOrgLogro(campo: CampoFormulario): string | null {
    return campo.value.trim() === '' ? 'Ingresa la organización o promotora.' : null;
  }

  function validarCampoAnioLogro(campo: CampoFormulario): string | null {
    const anioActual = new Date().getFullYear();
    if (campo.value.trim() === '') return 'Ingresa el año.';
  ...
  function validarCampoNivelLogro(campo: CampoFormulario): string | null {
    return campo.value === '' ? 'Selecciona el nivel competitivo.' : null;
  }

  function validarCampoLogro(campo: CampoFormulario, validar: (campo: CampoFormulario) => string | null): boolean {
  ...
  function revalidarCampoLogroSiYaTeniaError(campo: CampoFormulario, validar: (campo: CampoFormulario) => string | null): void {
    if (!obtenerContenedor(campo).querySelector('.error-campo')) return;
  ...
  function crearCampoLogro(idCampo: string, etiqueta: string, requerido: boolean): HTMLDivElement {
  ...
  function crearFilaLogro(indice: number): HTMLElement {
    ...
    function actualizarResumen(): void {
      ...
      const nivelTexto = selectNivel.options[selectNivel.selectedIndex]?.textContent ?? '';
  ...
  function filaEstaEnEdicion(fila: HTMLElement): boolean {
    const edicion = fila.querySelector<HTMLElement>('.logro-edicion');
  ...
  function mostrarErrorLogros(mensaje: string): void {
  ...
  function limpiarErrorLogros(): void {
    const contenedor = document.getElementById('logros');
    if (!contenedor) return;
  ...
  function revalidarLogrosSiYaTeniaError(): void {
    const contenedor = document.getElementById('logros');
    const lista = document.getElementById('logrosLista');
    if (!contenedor || !(lista instanceof HTMLElement) || !contenedor.querySelector(':scope > .error-campo')) return;

    const sigueHabiendoFilaSinGuardar = Array.from(lista.querySelectorAll<HTMLElement>('.logro-fila')).some((fila) =>
      filaEstaEnEdicion(fila),
    );
    if (!sigueHabiendoFilaSinGuardar) limpiarErrorLogros();
  }
  ...
  function validarFilaLogro(fila: HTMLElement): CampoFormulario | null {
    let primerInvalido: CampoFormulario | null = null;
    const registrar = (campo: HTMLElement | null, validar: (campo: CampoFormulario) => string | null) => {
      if (!esCampoValidable(campo)) return;
      const esValido = validarCampoLogro(campo, validar);
      if (!esValido && !primerInvalido) primerInvalido = campo;
    };

    registrar(fila.querySelector<HTMLElement>('[id^="achievementTitle_"]'), validarCampoTituloLogro);
    registrar(fila.querySelector<HTMLElement>('[id^="achievementOrg_"]'), validarCampoOrgLogro);
    registrar(fila.querySelector<HTMLElement>('[id^="achievementYear_"]'), validarCampoAnioLogro);
    registrar(fila.querySelector<HTMLElement>('[id^="achievementLevel_"]'), validarCampoNivelLogro);
  ...
  if (disciplineEl instanceof HTMLSelectElement && weightClassEl instanceof HTMLSelectElement) {
    disciplineEl.addEventListener('change', () => {
      ...
      if (!divisiones) {
        ...
        alternativeWeightClassesContenedor?.classList.add('oculto');
        return;
      }
      ...
      alternativeWeightClassesContenedor?.classList.remove('oculto');
    });
  }
  ...
  function alternarCampoCondicional(campo: HTMLInputElement, contenedor: HTMLElement, activo: boolean): void {
  ...
  formulario.addEventListener('submit', (evento: SubmitEvent) => {
    ...
    if (logrosListaEl instanceof HTMLElement) {
      const filaSinGuardar = Array.from(logrosListaEl.querySelectorAll<HTMLElement>('.logro-fila')).find((fila) =>
        filaEstaEnEdicion(fila),
      );
      if (filaSinGuardar) {
        mostrarErrorLogros('Termina de guardar tu logro antes de enviar.');
        if (!primerCampoInvalido) {
          const primerCampoDeLaFila = filaSinGuardar.querySelector<HTMLElement>('[id^="achievementTitle_"]');
          if (esCampoValidable(primerCampoDeLaFila)) primerCampoInvalido = primerCampoDeLaFila;
        }
      } else {
        limpiarErrorLogros();
      }
