...
(function initBuscarPeleadores() {
    var _a;
    const sidebar = document.querySelector('.buscar-sidebar');
    const chips = document.querySelectorAll('.chip-disciplina');
    ...
    function aplicarDisciplina(disciplina) {
        document.querySelectorAll('[data-disciplina-grupo]').forEach((grupo) => {
            grupo.classList.toggle('oculto', grupo.dataset.disciplinaGrupo !== disciplina);
        });
        document.querySelectorAll('[data-solo="mma"]').forEach((grupo) => {
    ...
    chips.forEach((chip) => {
        chip.addEventListener('click', () => {
            ...
            if (disciplina)
                aplicarDisciplina(disciplina);
        });
    });
    (_a = document.getElementById('limpiarFiltros')) === null || _a === void 0 ? void 0 : _a.addEventListener('click', () => {
        sidebar.querySelectorAll('input[type="text"], input[type="number"]').forEach((input) => {
            input.value = '';
        });
        ...
        sidebar.querySelectorAll('input[type="checkbox"]').forEach((checkbox) => {
            if (!checkbox.closest('.oculto'))
                checkbox.checked = false;
        });
    });
})();
//# sourceMappingURL=buscarPeleadores.js.map